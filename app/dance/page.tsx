...
import { Music, CheckCircle2, Award, Clock, Users, ArrowRight } from "lucide-react";

export const metadata = {
  title: "Dance Competition — VAJRA",
  description: "Express passion, rhythm and social storytelling on stage. Dance competition organized by SWB & NSS Unit, CMRCET.",
};

export default function DancePage() {
  const event = eventsMap["dance"];
  ...
  return (
    <div className="relative min-h-screen bg-navy-950">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-16 space-y-20">
        ...
        <section>
          <SectionDivider title="Performance Specifications" />
          <div className="grid sm:grid-cols-2 gap-6">
            <ScrollReveal delay={0.1}>
              <div className="glass-card p-6 rounded-2xl border border-gold-500/20 flex items-start gap-4">
                <div className="p-3 rounded-xl bg-gold-500/10 text-gold-400 shrink-0">
                  <Clock className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="font-serif text-lg font-bold text-gold-300 mb-1">Duration Limits</h3>
                  <p className="text-sm text-white font-semibold mb-1">{event.performanceDuration}</p>
                  <p className="text-xs text-slate-400">Includes setup and stage clearance time. Overtime penalties apply.</p>
                </div>
              </div>
            </ScrollReveal>
            ...
            <ScrollReveal delay={0.2}>
              <div className="glass-card p-6 rounded-2xl border border-gold-500/20 flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-serif text-lg font-bold text-gold-300 mb-1">Team Structure</h3>
                  <p className="text-sm text-white font-semibold mb-1">{event.teamSize}</p>
                  <p className="text-xs text-slate-400">All performers must be currently enrolled students with valid ID cards.</p>
                </div>
              </div>
            </ScrollReveal>
          </div>
        ...
        {/* Rules & Guidelines */}
        <section>
          <SectionDivider title="Stage Regulations" />
          <div className="text-center mb-10">
            <h2 className="font-serif text-3xl font-bold text-white mb-2">Dance Guidelines</h2>
            <p className="text-slate-400 text-sm">Strict compliance required for audio tracks, costumes, and props:</p>
          </div>
        ...
        {event.judgingCriteria && (
          <section>
            <SectionDivider title="Judging Parameters" />
            <div className="text-center mb-10">
              <h2 className="font-serif text-3xl font-bold text-white mb-2">Evaluation Criteria</h2>
              <p className="text-slate-400 text-sm">Judges will score performances based on:</p>
            </div>
        ...
        <section className="glass-card p-10 rounded-3xl border border-gold-500/40 text-center relative overflow-hidden shadow-[0_0_40px_rgba(212,175,55,0.15)]">
          ...
          <div className="relative z-10">
            <h2 className="font-serif text-3xl sm:text-4xl font-bold text-white mb-3">Own the Stage at VAJRA</h2>
            <p className="text-slate-300 text-base max-w-xl mx-auto mb-8">
              Register your dance team before slots fill up!
            </p>
